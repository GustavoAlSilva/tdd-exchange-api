...
describe('CurrenciesRepository', () => {
  ...
  describe('getCurrency()', () => {
    it('should be called findOne with correct params', async () => {
      ...
      expect(repository.findOne).toHaveBeenCalledWith({ where: { currency: 'USD' } });
    });

    it('should be throw findOne returns empty', async () => {
      repository.findOne = jest.fn().mockReturnValue(undefined);
      const currency = 'USD';
      await expect(repository.getCurrency(currency)).rejects.toThrow(
        new NotFoundException(`The currency ${currency} not found.`),
      );
    });
  ...
  describe('deleteCurrency()', () => {
    ...
    it('should be called delete with correct params', async () => {
      ...
      await repository.deleteCurrency('USD');
      expect(repository.delete).toHaveBeenCalledWith({ currency: 'USD' });
    });
